...
is-deeply $r.smembers('set-a').sort, ('a', 'b', 'c').sort, 'SMEMBERS';
...
is-deeply $r.sinter('set-a', 'set-b').sort, ('c', 'd').sort, 'SINTER';

ok $r.sinterstore('set-inter', 'set-a', 'set-b'), 'SINTERSTORE';

is-deeply $r.smembers('set-inter').sort, ('c', 'd').sort, 'Check';

is-deeply $r.sdiff('set-a', 'set-b').sort, ('a', 'b').sort, 'SDIFF';

ok $r.sdiffstore('set-diff', 'set-a', 'set-b'), 'SDIFFSTORE';

is-deeply $r.smembers('set-diff').sort, ('a', 'b').sort, 'Check';

is-deeply $r.sunion('set-a', 'set-b').sort, (<a b c d e f>).sort, 'SUNION';

ok $r.sunionstore('set-union', 'set-a', 'set-b'), 'SUNIONSTORE';

is-deeply $r.smembers('set-union').sort, <a b c d e f>.sort, 'Check';

ok $r.smove('set-a', 'set-b', 'a'), 'SMOVE';

is-deeply $r.smembers('set-a').sort, <b c d>.sort, 'A is right';

is-deeply $r.smembers('set-b').sort, <a c d e f>.sort, 'B is right';
